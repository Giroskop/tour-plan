	...
	$(this).validate({
		...
		messages: {
			...
			tel: {
				...
				minlength: 'minimum 11 characters needed',
			},
		},
	})
})
...
function endPoints() {
	if (parseInt(cardIntroduceHeight) > 175) {
		do {
			cardIntroduce.textContent = cardIntroduce.textContent.slice(
				0,
				cardIntroduce.textContent.length - 10
			)
		...
		while (
			...
			cardIntroduce.textContent = cardIntroduce.textContent.slice(
				0,
				cardIntroduce.textContent.length - 1
			)
		}
